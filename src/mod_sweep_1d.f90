...
contains
   ...
   subroutine temperature_sweep_1d(L, beta_min, beta_max, n_beta, &
      n_therm, n_steps, filename)

      integer(i4), intent(in) :: L, n_beta, n_therm, n_steps
      ...
      deallocate(beta_vals)
      deallocate(m_vals)
      deallocate(e_vals)

   end subroutine temperature_sweep_1d

end module mod_sweep_1d
